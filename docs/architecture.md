...
Light curtain safety image ---> Safety-to-MQTT gateway -----/
Vision inspection result -----> Inspection-to-MQTT gateway -/
```

The first simulator represents a small cobot machine-tending cell. It exposes a compact holding-register map with safety, production, and downtime signals. The second simulator represents a Profinet part feeder that sends cyclic process images. The third simulator represents a light curtain publishing safety state such as beam clear, OSSD outputs, muting, and reset requirements. The fourth simulator represents a smart camera publishing inspection pass/fail, confidence, and reject count. Each gateway reads the machine-facing protocol and publishes a normalized JSON message to MQTT.
